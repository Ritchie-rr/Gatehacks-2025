...
from dataloader import ASLDataModule  # use your existing dataloader

# ---------------- Device setup
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Using device:", device)

# ---------------- Load dataset to auto-detect SEQ_LEN and CLASSES
dm = ASLDataModule()
dm.setup()

# Detect SEQ_LEN from first batch
for x, _ in dm.train_dataloader():
    SEQ_LEN = x.shape[1]  # sequence length dimension
    break
print("Detected SEQ_LEN:", SEQ_LEN)

# Detect classes dynamically
CLASSES = sorted(dm.train_set.label_to_idx, key=lambda k: dm.train_set.label_to_idx[k])
...
def preprocess_frame(landmarks):
    """
    Convert single frame's hand landmarks into LSTM input
    """
    global frame_buffer
    ...
    seq = np.array(frame_buffer)  # shape: (SEQ_LEN, num_features)
    seq_tensor = torch.tensor(seq, dtype=torch.float32).unsqueeze(0).to(device)  # (1, SEQ_LEN, num_features)
